...
- `sql_parse.py` parses one SQL Program with SQLGlot and preserves assertion
  markers.
- `ir/model.py` contains the immutable bound semantic model.
- `ir/convert.py` is the only boundary that reads SQLGlot ASTs and lowers them
  into that model.
- `engine.py` adds facts and rules to Clingo, grounds them, solves, and enforces
...
entire analysis. The suffix establishes identity; the hint is diagnostic only.

- Relation Definitions and Relation Instances use `rel`, for example
  `rel_users_1` and `rel_customer_2`.
- Bound column references use `col`, for example `col_id_3`.
